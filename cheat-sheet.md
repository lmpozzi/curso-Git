...
| `git cherry-pick <hash>`      | Aplica **un commit** de otra rama sobre la actual   |
| `git cherry-pick --abort`     | Cancela si hay conflicto                            |
...
| `git merge origin/<rama>`     | Fusiona una rama remota (después de `fetch`)                                      |

**Flujo con Pull Request:** crear rama → commits → `git push -u origin <rama>` → abrir PR en GitHub → revisión → merge → `git checkout main` + `git pull`.

## Sub-repositorios (submódulos)

```bash
git submodule add <url> libs/nombre           # agrega un repo dentro del tuyo
git clone --recurse-submodules <url>          # clona incluyendo submódulos
git submodule update --init --recursive       # si clonaste sin el flag y la carpeta quedó vacía
git submodule update --remote                 # actualiza al último commit del submódulo
```
